...
+ function ($) {
    ...
    $.modal = function (params, onOpen) {
        ...
        var closebtnHtml = buttons.map(function (d, i) {
            return '<a href="javascript:;" style="color:white" class="weui-dialog__btn ' + (d.className || "") + '">' + "确认" + '</a>';
        }).join("");
        var tpl = '<div class="weui-dialog">' +
                    '<div class="weui-dialog__hd" ><strong class="weui-dialog__title">' + params.title + '</strong></div>';
        if (params.isOk)
            tpl += (params.text ? '<div class="weui-dialog__bd" style="margin-bottom: 44px;" >' + params.text + '</div>' : '');
        else
            tpl += (params.text ? '<div class="weui-dialog__bd"  >' + params.text + '</div>' : '');
        tpl+='<div class="weui-dialog__ft">' + buttonsHtml + '</div>';
        if (params.isOk) {
            tpl += '<div class="weui-dialog__fter">' + closebtnHtml + '</div>'
        }
        tpl += '</div>';

        var dialog = $.openModal(tpl, onOpen);
        var btns = params.isOk ? buttons.concat(closebtn) : buttons;
        dialog.find(".weui-dialog__btn").each(function (i, e) {
            ...
            el.click(function () {
                ...
                console.log(i);
                if (btns[i] && btns[i].onClick) {
                    btns[i].onClick.call(dialog);
                }
            });
        });
